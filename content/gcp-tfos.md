name: Terraform-Vault-Workshop
class: center,middle,title-slide
count: false
![:scale 70%](../../../css/images/TerraformLogo.png)
![:scale 40%](../../../css/images/gpc/GCPLogo.png)
.titletext[
...
**If you don't have a favorite text editor, that's okay! We've brought prebuilt cloud workstations that have Visual Studio Code already preinstalled. VSC is a free programmer's text editor for GCP, and it has great Terraform support. Most of this workshop will be simply copying and pasting code, so if you're not a developer don't fret. Terraform is easy to learn and fun to work with.**
...
5. Provision and Configure GCP VMs
...
Method 1: GUI (GCP Console)
-------------------------
![:scale 100%](../../../css/images/gcp/GCP-Portal.png)

???
**This should look familiar if you've ever used GCP. You click on Virtual Machines, and you'll see a whole list of different base images you can use to provision your VM. Some of these are provided by Amazon, others are provided by third parties in the marketplace. You either search or browse for the thing you need, and click on it. Easy.**

---
name: GCP-Cloud-Deployment-Manager
Method 2: GCP Cloud Deployment Manager (CDM)
-------------------------
```yaml
resources:
- name: the-first-vm
...
CDM configurations provide a consistent and reliable way to provision GCP resources. YAML is easy for computers to read, but can be challenging for humans to edit and troubleshoot.

???
**Which brings us to method #2, GCP Cloud Deployment Manager configurations, also known as CDM configurations. Have any of you used CDM configurations? What's that experience like?**

**CDM configurations are written in YAML, which stands for Yet Another Markup Language. It is commonly used for configuration files and in applications where data is being stored or transmitted. And don't get me wrong, YAML is great. If you happen to be a computer. Computers are really good at reading these files full of key-value pairs and lists.**

**The problem is that editing and maintaining huge YAML files is hard for humans. Because YAML is not a programming language, you'll end up writing a lot more lines of complex code that is hard to understand and change.**
...
* Change and update existing infrastructure
* Safely test changes using **`terraform plan`** in dry run mode
* Integrate with application code workflows (Git, GCP DevOps, CI/CD tools)
]
...
-------------------------
<br><br>
.center[![:scale 60%](../../../css/images/infra_tools.png)]
...
-------------------------
<br><br>
.center[![:scale 100%](../../../css/images/gcp/gcp-cloud.png)]
...
---
name: Config-Hell
.center[![:scale 90%](../../../css/images/Config_Hell.jpg)]
???
...
labels:
    - key: Name
      value: context.env["name"] + '-vm'
    - key: Description
      value: context.env["description"]
```
Terraform:
```hcl
tags {
...
Terraform code (HCL) is easy to learn and easy to read. No issues with white space and can be up to 50-70% more compact than an equivalent YAML configuration.
...
???
**Terraform is also great for migrating between cloud providers. Let's say you wanted to move some workloads from GCP to GCP. The code changes in Terraform would be much easier to implement than they would via ARM templates. I was able to migrate a simple demo application from one cloud to another in a few short hours, because there was almost no learning curve. Terraform code looks the same no matter where you run it.**
...
???
Instructor Note: If your users want to bring their own GCP accounts, they can do all the exercises in GCP Cloudshell. Or if you are able to spin up accounts inside a training account and provide them to your students that's fine too. Otherwise we recommend using the GCP Dev/Test prebuilt workstations described in the instructor notes in this repo.
...
Run the setup_gcp.ps1 script
-------------------------
<br><br>
.center[![:scale 50%](../../../css/images/run_setup.png)]
...
**WARNING:** Do not skip this step. It is required to set up your connection to GCP Cloud.

???
If anyone is curious what this powershell script does, it's disabling windows line endings for git clone. It also fetches dynamic GCP credentials that are good for 8 hours.

**This handy script does some setup and fetches dynamic GCP credentials from our training Vault server. Right click on the setup.ps1 file and select the "Run with Powershell" option. It may take a minute or two to finish.**
...
-------------------------
<br><br>
.center[![:scale 80%](../../../css/images/ready_to_terraform.png)]
...
-------------------------
.center[![:scale 70%](../../../css/images/set_colors_and_icons.png)]
...
-------------------------
<br><br>
.center[![:scale 70%](../../../css/images/renderer.png)]
...
-------------------------
.center[![:scale 70%](../../../css/images/install_terraform_extension.png)]
...
-------------------------
.center[![:scale 70%](../../../css/images/open_terminal.png)]
...
-------------------------
Run the following commands to clone the training repository from GitHub. Run the Windows workstation commands if you have a cloud workstation provided by your instructor. Run the Cloudshell commands if you're using GCP Cloudshell in a browser window.
...
-------------------------
.center[![:scale 70%](../../../css/images/vsc_tour.png)]
...
-------------------------
.center[![:scale 70%](../../../css/images/terraform_config_files.png)]
...
- Downloading plugin for provider "GCP" (2.17.0)...
...
**If you're curious, how are we authenticating to GCP? We saved some GCP credentials on your workstation as environment variables when we ran the setup.ps1 script. You can also use Terraform directly from GCP cloudshell. Terraform is preinstalled in cloudshell and doesn't require any authentication or special configuration.**
...
NOTE TO INSTRUCTOR: If students have a very common name, they should add a number to the end of it to guarantee that it is available. The default GCP public 'cloudapp' DNS zone is a global namespace. If your student enters a very common word here, there's a chance it could be taken already. This will cause the Terraform run to fail.
...
Choose the GCP location nearest to you and set the 'region' variable. You can find a list of GCP regions here:
...
.center[![:scale 80%](../../../css/images/resource_anatomy.png)]

*resource* = top level keyword

*type* = this is the name of the resource. The first part tells you which provider it belongs to. Example: `google_compute_instance`. This means the provider is GCP and the specific type of resource is a virtual machine.
...
GCP_vpc.workshop: Creating...
  arn:                              "" => "<computed>"
  assign_generated_ipv6_cidr_block: "" => "false"
  cidr_block:                       "" => "10.0.0.0/16"
  ....
GCP_vpc.workshop: Creation complete after 8s (ID: vpc-0cf22f733192....)
...
GCP_vpc.workshop: Refreshing state... (ID: vpc-0cf22f733192....)
...
<br><br><br>
Read the documentation for the `google_compute_network` resource and learn how to add labels to the vpc:

https://www.terraform.io/docs/providers/google/r/compute_network.html

Edit your main.tf file and add a label to the resource. Set the name of the label to 'environment' and the value to 'Production'.
...
```tex
AGCP_vpc.workshop: Modifying... (ID: vpc-01a5c1289101109b6)
  tags.%:           "1" => "2"
  tags.environment: "" => "Production"
GCP_vpc.workshop: Modifications complete after 6s (ID: vpc-01a5c1289101109b6)
```
...
-------------------------
<br><br>
Let's add a subnet. Scroll down in the main.tf file until you find the `GCP_subnet` resource. Uncomment it and save the file.

```terraform
resource "GCP_subnet" "subnet" {
  vpc_id     = "${GCP_vpc.workshop.id}"
...
-------------------------
<br><br>
Terraform can automatically keep track of dependencies for you. Let's take a look at the two resources in our main.tf file. Note the highlighted line in the GCP_subnet resource. This is how we tell one resource to refer to another in terraform.

```terraform
resource "GCP_vpc" "workshop" {
  cidr_block       = "10.0.0.0/16"
  instance_tenancy = "dedicated"
  tags = {
    Name = "${var.prefix}-workshop"
...
Output:
```tex
GCP_vpc.workshop: Refreshing state... (ID: vpc-01a5c1289101109b6)
GCP_subnet.subnet: Creating...
  arn:                             "" => "<computed>"
...
GCP_subnet.subnet: Creation complete after 3s (ID: subnet-0604cc348237cc9be)
...
-------------------------
.center[![:scale 85%](../../../css/images/terraform_config_files.png)]
...
variable "region" {
  description = "The region of GCP to use"
  default     = "us-east-1"
}
...
output "Vault_Server_URL" {
  value = "http://${GCP_instance.vault-server.public_ip}:8200"
}

output "MySQL_Server_FQDN" {
  value = "${GCP_db_instance.vault-demo.address}"
}
...
ssh ubuntu@${GCP_instance.vault-server.public_ip}
...
MySQL_Server_FQDN = terraform-20190627032621775400000001.coydleaeb0pt.us-east-1.rds.amazonGCP.com
...
<br><br><br><br>
Take the GCP_instance resource out of main.tf and put it into its own file called **vm.tf**. Save both files. Run **`terraform apply`** again. What happens?
...
connection {
  ...
  host = "${GCP_instance.vault-server.public_ip}"
}
provisioner "file" {
  source      = "files/"
...
-------------------------
.center[![:scale 80%](../../../css/images/cpa.jpg)]
...
```bash
terraform taint GCP_instance.vault-server
terraform apply -auto-approve
```
...
<br>
.center[![:scale 100%](../../../css/images/lab6-question.png)]
...
-------------------------
<br><br><br>
You can try the [Introduction to Vault](workshop.html?content=gcp-vault.md) Workshop, or proceed to the [Intro to Terraform Enterprise](workshop.html?content=gcp-tfe.md) Workshop.
...
[Introduction to Vault](workshop.html?content=gcp-vault.md) - Learn the Basics of HashiCorp Vault

[Intro to Terraform Enterprise](workshop.html?content=gcp-tfe.md) - Explore Terraform Cloud and Enterprise
